...
train_set = datasets.MNIST(root=data_path,train=True,
                           transform=transforms.Compose([transforms.ToTensor(),transform_flatten,
...
class NewBatchNormv1(nn.Module):
    ...
    def __init__(self, num_features, max_length, eps=1e-5, momentum=0.9,
                 affine=True):
        """
        ...
        super(NewBatchNormv1, self).__init__()
...
class NewBatchNorm1D(nn.Module):
    ...
    def __init__(self, num_features, max_length, eps=1e-5, momentum=0.9, affine=True, use_bias=False, dev_measure='SQD', alpha=0.25):
    ...
    def forward(self, input, time):
        ...
        if self.affine:
            if self.use_bias:
                ...
                bn_output = self.weight * (input - tmp_mean) / (tmp_var + self.eps)
        else:
...
class NewBatchNorm(nn.Module):
    ...
    def forward(self, input_, time):  ##目前的input是一个（32, 400）的矩阵
        ...
        if self.affine:
            if self.use_bias:
                bn_output = self.weight * ((input_ - running_mean_i + self.eps) / (running_var_i + self.eps)) + self.bias
                ...
                # d = b / c.view(-1,1)
                # ###################
                bn_output = self.weight * ((input_ - running_mean_i + self.eps) / (running_var_i + self.eps))
                #bn_output = (input_ - running_mean_i.view(-1, 1)) / (running_var_i + self.eps).view(-1, 1)
        else:
            bn_output = ((input_ - running_mean_i + self.eps) / (running_var_i + self.eps))
...
class LSTMCell(nn.Module):
    def __init__(self, input_size, hidden_size, max_length, use_bias=True, momentum=0.9, dev_measure='SD', alpha=0.25):
        ...
        # self.bn_h = NewBatchNormv1(num_features=hidden_size * 4, max_length=784, eps=1e-5, momentum=0.1)
        # self.bn_i = NewBatchNormv1(num_features=hidden_size * 4, max_length=784, eps=1e-5, momentum=0.1)
        # self.bn_c = NewBatchNormv1(num_features=hidden_size, max_length=784, eps=1e-5, momentum=0.1)


        self.bn_h = NewBatchNorm1D(num_features=hidden_size * 4, max_length=784, affine=True, use_bias=False, dev_measure='SQD', alpha=0.25)
        self.bn_i = NewBatchNorm1D(num_features=hidden_size * 4, max_length=784, affine=True, use_bias=False, dev_measure='SQD', alpha=0.25)
        self.bn_c = NewBatchNorm1D(num_features=hidden_size, max_length=784, affine=True, use_bias=True, dev_measure='SQD',alpha=0.25)
    ...
    def reset_parameters(self):
        ...
        # self.bn_i.reset_parameters()
        # self.bn_h.reset_parameters()
    ...
    #     f, i, o, g = torch.split(bn_h + bn_i + bias_batch,split_size_or_sections=self.hidden_size, dim=1)
    #     # print('f',f.size())
    #     # print('i',i.size())
    #     # print('o',o.size())
    #     # print('g',g.size())
    ...
    def forward(self, input_, hx, time):
        ...
        wh = torch.mm(h_0, self.weight_hidden)
        wi = torch.mm(input_, self.weight_input)
        f, i, o, g = torch.split(wh + wi + bias_batch,split_size_or_sections=self.hidden_size, dim=1)
        c_1 = torch.sigmoid(f) * c_0 + torch.sigmoid(i) * torch.tanh(g)
...
class LSTM(nn.Module):
    def __init__(self, cell_class, input_size, hidden_size, max_length, num_layers=2, use_bias=True, momentum=0.9,
                 dropout=0.5, dev_measure='SD',alpha=0.25):
    ...
    def forward(self, input_, hx):
        ...
        print("cell_0 hidden parameter",self.cell_0.weight_hidden)
        # print("cell_0 input parameter",self.cell_0.weight_input)
        # print("cell_1 hidden parameter",self.cell_1.weight_hidden)
        # print("cell_1 input parameter",self.cell_1.weight_input)
...
for epoch in range(epochs):
    ...
    for data, label in train_loader:
        data = Variable(data.view(-1,batch_size)+1)
